...
pm1 <- proc.time()
library(csrnaseq)
covset$RFI <- with(covset, RFI - ave(RFI, Line))
FixCov0 <- covset["Line"]
...
RealDataOut <- NULL
dir.create(path = "revise/RealDataOutBSRFIprojectiononLine", showWarnings = F, recursive = T)
for (i in names(Config)){
  ...
  saveRDS(RealDataOut, file = paste0("revise/RealDataOutBSRFIprojectiononLine/", i, ".rds")) # use this if run on Wahba HPC
}
